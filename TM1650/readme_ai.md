...
|------------|------------|--------------------------|------------|----------------|
| `tm1650_init_hardware` | Statement | VAR(field_input), DIGITS(field_number), BRIGHTNESS(input_value) | `tm1650_init_hardware("display", 4, math_number(3))` | `display.init(); ↵ display.setBrightness(constrain(1, 0, 7));` |
| `tm1650_init` | Statement | VAR(field_input), DIGITS(field_number), SDA(input_value), SCL(input_value), BRIGHTNESS(input_value) | `tm1650_init("display", 4, math_number(8), math_number(9), math_number(3))` | `display.init(); ↵ display.setBrightness(constrain(1, 0, 7));` |
| `tm1650_show_text` | Statement | VAR(field_variable), TEXT(input_value) | `tm1650_show_text($display, text("12.34"))` | `display.displayValue(String("value"));` |
| `tm1650_show_number` | Statement | VAR(field_variable), NUMBER(input_value), DECIMALS(field_number) | `tm1650_show_number($display, math_number(12.34), 2)` | `display.displayNumber(1, 0);` |
| `tm1650_clear` | Statement | VAR(field_variable) | `tm1650_clear($display)` | `display.clear();` |
| `tm1650_set_dot` | Statement | VAR(field_variable), POSITION(input_value), STATE(input_value) | `tm1650_set_dot($display, math_number(2), logic_boolean(TRUE))` | `display.setDecimalPoint(1, true);` |
| `tm1650_set_brightness` | Statement | VAR(field_variable), MODE(dropdown), BRIGHTNESS(input_value) | `tm1650_set_brightness($display, IMMEDIATE, math_number(3))` | `display.setBrightness(constrain(1, 0, 7));` |
| `tm1650_set_power` | Statement | VAR(field_variable), STATE(input_value) | `tm1650_set_power($display, logic_boolean(TRUE))` | `display.displayState(true);` |
| `tm1650_scroll_start` | Value Number | VAR(field_variable), TEXT(input_value) | `tm1650_scroll_start($display, text("HELLO"))` | `display.startScroll(String("value"))` |
| `tm1650_scroll_next` | Value Number | VAR(field_variable) | `tm1650_scroll_next($display)` | `display.scrollNext()` |
| `tm1650_scroll_text` | Statement | VAR(field_variable), TEXT(input_value), DELAY(input_value) | `tm1650_scroll_text($display, text("HELLO"), math_number(500))` | `ailyTm1650Scroll(display, String("value"), 1);` |
| `tm1650_set_segments` | Statement | VAR(field_variable), POSITION(input_value), SEGMENTS(input_value) | `tm1650_set_segments($display, math_number(1), math_number(63))` | `display.setSegments(1, 1);` |
| `tm1650_set_control` | Statement | VAR(field_variable), POSITION(input_value), VALUE(input_value) | `tm1650_set_control($display, math_number(1), math_number(49))` | `display.setControl(1, 1);` |
| `tm1650_get_segments` | Value Number | VAR(field_variable), POSITION(input_value) | `tm1650_get_segments($display, math_number(1))` | `display.getSegments(1)` |

...
|------------|------------|--------------------------|------------|----------------|
| `max31865_init` | Statement | VAR(field_input), SPI_MODE(dropdown), CS_PIN(dropdown), WIRES(dropdown); runtime variants: hardware-spi: (none); software-spi: SW_SCK_PIN(dropdown), SW_MOSI_PIN(dropdown), SW_MISO_PIN(dropdown) | `max31865_init("rtd", HW, 5, MAX31865_2WIRE)` | `Adafruit_MAX31865 rtd(CS_PIN); ↵ SPI.begin(); ↵ rtd.begin(MAX31865_2WIRE);` |
| `max31865_read_temperature` | Value(Number) | VAR(field_variable), RTD_NOMINAL(input_value), REF_RESISTOR(input_value) | `max31865_read_temperature($rtd, math_number(100), math_number(430))` | `rtd.temperature(1, 1)` |
...
| `max31865_set_wires` | Statement | VAR(field_variable), WIRES(dropdown) | `max31865_set_wires($rtd, MAX31865_3WIRE)` | `rtd.setWires(MAX31865_2WIRE);` |
...
arduino_loop()
    ...
    serial_println(Serial, max31865_read_temperature($rtd, math_number(100), math_number(430)))
    controls_if()
        @IF0: logic_compare(max31865_read_fault($rtd), NEQ, math_number(0))
        @DO0:
...
arduino_loop()
    serial_println(Serial, max31865_read_temperature($rtd, math_number(1000), math_number(4300)))
    time_delay(math_number(500))

## Notes

1. **Variable**: `max31865_init("varName", ...)` creates variable `$varName` of type `Adafruit_MAX31865`; pass `$varName` directly to `field_variable` slots; use `variables_get($varName)` only for `input_value` slots.
...
### Software SPI Runtime Variant

```abs
arduino_setup()
    max31865_init("softRtd", SW, 15, MAX31865_3WIRE, SW_SCK_PIN=18, SW_MOSI_PIN=23, SW_MISO_PIN=19)
```

...
// 初始化软串口
Arduino.forBlock['esp8266_softwareserial_init'] = function(block, generator) {
  // 变量重命名监听
...
// 检查是否有数据可读
Arduino.forBlock['esp8266_softwareserial_available'] = function(block, generator) {
  const varField = block.getField('VAR');
...
// 读取数据
Arduino.forBlock['esp8266_softwareserial_read'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 输出数据（不换行）
Arduino.forBlock['esp8266_softwareserial_print'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 输出数据（换行）
Arduino.forBlock['esp8266_softwareserial_println'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
  const data = generator.valueToCode(block, 'DATA', generator.ORDER_ATOMIC) || '""';
...
// 输出原始字节数据
Arduino.forBlock['esp8266_softwareserial_write'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 设置为活动监听端口
Arduino.forBlock['esp8266_softwareserial_listen'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 检查是否正在监听
Arduino.forBlock['esp8266_softwareserial_islistening'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 检查缓冲区溢出
Arduino.forBlock['esp8266_softwareserial_overflow'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';
...
// 关闭软串口
Arduino.forBlock['esp8266_softwareserial_end'] = function(block, generator) {
  const varField = block.getField('VAR');
  const varName = varField ? varField.getText() : 'mySerial';

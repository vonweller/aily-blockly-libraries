...
| `async_fs_webserver_connect_or_ap` | S | VAR(field_variable), TIMEOUT(input_value), SSID(input_value), PASSWORD(input_value), REDIRECT(input_value) | `async_fs_webserver_connect_or_ap($server, math_number(1000), text("value"), text("value"), text("value"))` | `if (!server.startWiFi(1000)) { ↵ server.startCaptivePortal("value", "value", "value"); ↵ }` |
| `async_fs_webserver_start_wifi` | S | VAR(field_variable), TIMEOUT(input_value) | `async_fs_webserver_start_wifi($server, math_number(1000))` | `server.startWiFi(1000);` |
...
| `async_fs_webserver_start_ws_server` | S | VAR(field_variable), HANDLER(input_statement) | `async_fs_webserver_start_ws_server($server)` | `server.init(asyncfs_ws_event);` |
...
| `async_fs_webserver_option_number` | S | VAR(field_variable), LABEL(input_value), VALUE(input_value), MIN(input_value), MAX(input_value), STEP(input_value) | `async_fs_webserver_option_number($server, text("value"), math_number(0), math_number(0), math_number(0), math_number(0))` | `server.addOption("value", 0, 0, 0, 1);` |
...
| `async_fs_webserver_route` | S | VAR(field_variable), PATH(input_value), METHOD(dropdown), HANDLER(input_statement) | `async_fs_webserver_route($server, text("value"), HTTP_GET)` | `server.on("value", HTTP_GET, asyncfs_route);` |
| `async_fs_webserver_response` | S | STATUS(input_value), CONTENT_TYPE(input_value), BODY(input_value) | `async_fs_webserver_response(math_number(0), text("value"), text("value"))` | `if (_asyncfs_request != nullptr) { ↵ _asyncfs_request->send(0, "value", "value"); ↵ }` |
...
### Filesystem, connection and route

```abs
arduino_setup()
    async_fs_webserver_create("server", SPIFFS, 8080, text("weather"))
    async_fs_webserver_mount_fs($server, SPIFFS, TRUE)
    async_fs_webserver_connect_or_ap($server, math_number(10000), text("ESP_AP"), text("123456789"), text("/setup"))
    async_fs_webserver_route($server, text("/health"), HTTP_GET)
        async_fs_webserver_response_ok()
    async_fs_webserver_start_server($server)
```

`async_fs_webserver_create` owns the `$server` model. Reuse that name in field-variable slots; references do not create additional models. The explicit create block determines filesystem/port/host, even when another block is visited first during generation. Regeneration always recreates declarations for the current workspace; callback names are generated, not user-facing ABS parameters. Network behavior still requires board testing.

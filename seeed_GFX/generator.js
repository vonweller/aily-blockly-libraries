...
function seeedGfxGetAnimationData(block) {
  ...
  if (typeof animationData === 'string') {
    try {
      ...
      throw new Error(`[seeed_gfx_animation] Failed to parse CUSTOM_ANIMATION: ${error}`);
    }
  }
  ...
  if (animationData.schemaVersion !== 1 || (!isRgb565 && !isRgb332)) {
    throw new Error('[seeed_gfx_animation] CUSTOM_ANIMATION is not a compact TFT animation state');
  }
  ...
  if (!Number.isInteger(width) || width <= 0 || width > 65535 ||
    ...
    throw new Error('[seeed_gfx_animation] Width and height must be positive 16-bit integers');
  }
  if (!Number.isFinite(fps) || fps <= 0) {
    throw new Error('[seeed_gfx_animation] FPS must be a positive number');
  }
  if (!Number.isInteger(frameCount) || frameCount < 0 || frameCount > 65535) {
    throw new Error('[seeed_gfx_animation] Animation frame count is invalid');
  }
  if (frameCount === 0 && !animationData.frames) {
    return null;
  }
  if (frameCount === 0 || !animationData.frames) {
    throw new Error('[seeed_gfx_animation] Animation frame reference and frame count are inconsistent');
  }
  ...
  if (!Number.isSafeInteger(expectedByteLength)) {
    throw new Error('[seeed_gfx_animation] Animation dimensions are too large');
  }

  const runtime = globalThis.ailyProjectData;
  if (!runtime || typeof runtime.getPreparedFieldPayload !== 'function') {
    throw new Error('[seeed_gfx_animation] Project Data runtime is unavailable');
  }
  ...
  if (!(packed instanceof Uint8Array) || packed.byteLength !== expectedByteLength) {
    throw new Error('[seeed_gfx_animation] Prepared animation payload length is invalid');
  }
  ...
  if (typeof resourceId !== 'string' || resourceId.length === 0) {
    throw new Error('[seeed_gfx_animation] Animation resource reference is invalid');
  }
  ...
  return {
    ...
    fps,
    format,
    signature: seeedGfxGetAnimationSignature(format, encoding, width, height, fps, [resourceId]),
    frames
  };
}
...
function seeedGfxGetImageData(block) {
  ...
  if (typeof imageData === 'string') {
    try {
      ...
      throw new Error(`[seeed_gfx_image] Failed to parse CUSTOM_IMAGE: ${error}`);
    }
  }
  ...
  if (imageData.schemaVersion !== 1 || (!isRgb565 && !isRgb332)) {
    throw new Error('[seeed_gfx_image] CUSTOM_IMAGE is not a compact TFT image state');
  }
  ...
  if (!Number.isInteger(width) || width <= 0 || width > 65535 ||
      !Number.isInteger(height) || height <= 0 || height > 65535) {
    throw new Error('[seeed_gfx_image] Width and height must be positive 16-bit integers');
  }
  ...
  if (frameCount !== 1 || !imageData.frames) {
    throw new Error('[seeed_gfx_image] Static image must contain exactly one prepared frame');
  }

  const expectedByteLength = width * height * (isRgb332 ? 1 : 2);
  if (!Number.isSafeInteger(expectedByteLength)) {
    throw new Error('[seeed_gfx_image] Image dimensions are too large');
  }
  const runtime = globalThis.ailyProjectData;
  if (!runtime || typeof runtime.getPreparedFieldPayload !== 'function') {
    throw new Error('[seeed_gfx_image] Project Data runtime is unavailable');
  }
  ...
  if (!(bytes instanceof Uint8Array) || bytes.byteLength !== expectedByteLength) {
    throw new Error('[seeed_gfx_image] Prepared image payload length is invalid');
  }
  const resourceId = imageData.frames?.$ailyData?.id;
  if (typeof resourceId !== 'string' || resourceId.length === 0) {
    throw new Error('[seeed_gfx_image] Image resource reference is invalid');
  }

...
function ensureEs8311QwenSupport(generator) {
  ...
  // Route ArduinoJson parsing to PSRAM so it never competes with TLS for the
  // scarce internal heap. Without this, audio-bearing SSE deltas fail to parse
  // (NoMemory) after the TLS handshake has consumed internal SRAM.
  generator.addFunction('es8311_qwen_psram_allocator', String.raw`
struct Es8311PsramAllocator : ArduinoJson::Allocator {
...
static Es8311PsramAllocator es8311_qwen_psram_allocator;`);
...
bool es8311_qwen_play_audio_chunk(ES8311Audio &audio, const char* base64Data) {
  ...
  if (requiredLength > decodedCapacity) {
    uint8_t* newBuffer = (uint8_t*)heap_caps_realloc(decodedBuffer, requiredLength, MALLOC_CAP_SPIRAM);
    if (newBuffer == nullptr) newBuffer = (uint8_t*)realloc(decodedBuffer, requiredLength);
...
String es8311_qwen_audio_chat_request(ES8311Audio &audio, String model, String prompt, String voice, bool backgroundPlay) {
  ...
  char* lineBuf = (char*)heap_caps_malloc(lineCap, MALLOC_CAP_SPIRAM);
  if (lineBuf == nullptr) lineBuf = (char*)malloc(lineCap);
  if (lineBuf == nullptr) {
    client.stop();
    audio.streamEnd();
  ...
  while (!streamDone && (client.connected() || client.available())) {
    ...
    for (int bi = 0; bi < n; bi++) {
      ...
      if (ch != '\n') {
        if (lineLen >= lineCap - 1 && !lineOverflow) {
          if (lineCap < LINE_CAP_MAX) {
            ...
            char* newBuf = (char*)heap_caps_realloc(lineBuf, newCap, MALLOC_CAP_SPIRAM);
            if (newBuf == nullptr) newBuf = (char*)realloc(lineBuf, newCap);
            if (newBuf != nullptr) {
              lineBuf = newBuf;
              lineCap = newCap;
            } else {
      ...
      JsonDocument doc(&es8311_qwen_psram_allocator);
      DeserializationError jsonError = deserializeJson(doc, dataPtr);
      if (jsonError) {
        Serial.print("[ES8311] SSE parse fail: ");
        Serial.print(jsonError.c_str());
        Serial.print(" heap=");
...
Arduino.forBlock['es8311_init'] = function(block, generator) {
  ...
  // OJoy 板级: 功放 CTRL=IO9, 默认下拉关闭, 必须拉高才有声音输出(通用 ES8311 库不含此步)
  generator.addSetup('ojoy_amp_enable', 'pinMode(9, OUTPUT); digitalWrite(9, HIGH); // OJoy 使能功放 (CTRL=IO9)');

  // OJoy 音频引脚固定: I2C SDA=5 SCL=4 (0x18自动扫描), I2S MCLK=6 BCK=14 WS=12 dout=11(→ES8311 DSDIN) din=13(←ES8311 ASDOUT)
  return varName + '.begin(5, 4, 6, 14, 12, 11, 13, ' + duration + ');\n';
};

// es8311_i2s_config 已移除: I2S 引脚已在 OJoy 初始化块中定死
